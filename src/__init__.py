...
app = Flask(__name__) #η library Flask είναι WEB Framework, it is used for developing web applications using python (δες σχόλια επάνω)
jwt = JWTManager(app) #εδώ είναι τα JWT που είναι cookies (είναι jwt cookies, που είναι διαφορετικά από τα session cookies) και κάνουν το authentication του client που συνδέετεται με την ΒΔ (εν προκειμένω, στην εφαρμογή μας)
...
cors = CORS(
    app,
    resources={r"*": {"origins": ["http://localhost:4200"]}}, #στην port 4200 τρέχει η angular, εδώ του λέμε να επιτρέπεται ό,τι τρέχει τοπικά σε Angular
)

app.register_blueprint(user, url_prefix="/user") #εδώ του λέμε ότι στο endpoint /user έχουμε πρόσβαση στο blueprint user το οποίο με την σειρά του έχει τα δικά του endpoints => κι έτσι θα έχουμε πρόσβαση στα endpoints του blueprint user 'χτυπώντας' τα: /user/register, /user/check_duplicate_email/<string:email> Κλπ

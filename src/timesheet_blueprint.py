...
def get_timesheets_by_date(dateOfWork):
    try:
        timesheet = Timesheet.objects(dateOfWork=dateOfWork).first()
        if timesheet:
            return Response(json.dumps(timesheet.to_mongo()), status=200)
        return Response(json.dumps({"msg": "Timesheet for given date not found"}), status=404)
    except Exception as e:
        print(e)
        return Response(json.dumps({"msg": str(e)}), status=400)
...
@timesheet.route("/afm/<string:afm>", methods=["PATCH"])
def update_timesheet_by_id(id):
    try:
        data = request.get_json() 
        # data = request.get_json # = problem for update
        print(data)
        Timesheet.objects(_id=id).update_one(**data)
...
def delete_timesheet(id):
    try:
        ...
        data = request.get_json        
        Timesheet.objects(_id=id).delete()        
